...
const Questions = () => {
  ...
  const faqData = [
    ...
    {
      question: 'What is the minimum amount needed to trade on AlfateksTrade Trade?',
      answer: 'We offer $50 as the minimum deposit. On the AlfateksTrade Trade trading platform, trades are made by dollar amount, rather than by unit of an asset. That means that you don’t need to buy whole units of an asset; you can buy just a fraction of a cryptocurrency, starting from a minimum of $50 per trade.',
    },
    {
      question: 'Which payment methods are accepted for deposits?',
      answer: 'We currently accept deposits through Crypto Deposits, PayPal, or wire transfer only. These payment methods are the best options for secure deposits and withdrawals, protecting our clients and ensuring that all funds are properly accounted for',
  ...
  return (
    <div className='mt-[16%]'>
      <div className="items-center justify-center text-center pt-8">
        <h3 className='text-[#B6F848] text-3xl'>QUESTION</h3>
        <p className='text-white text-5xl pt-9'>The most frequently asked questions</p>
      </div>

      <div className="p-2 mt-9 text-white">
        {faqData.map((faq, index) => (
          <div
            key={index}
            className={`bg-[#191919] rounded-lg cursor-pointer p-6 mb-7 hover:text-[#B6F848] ${
              openIndex === index ? 'bg-[#B6F848] text-black' : ''
            }`}
            onClick={() => toggleFaq(index)}
          >
            <div className="flex justify-between items-center">
              <h2 className={`text-extrabold font-semibold ${openIndex === index ? 'text-black' : ''}`}>
                {faq.question}
              </h2>
